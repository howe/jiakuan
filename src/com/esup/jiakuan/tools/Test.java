...
public class Test extends EntityService<Object> {
	...
	public void t() throws Exception {
		...
		List<Trade> list = queryTradeList();
		...
		if (list != null) {
			for (Trade trade : list) {
				...
				if (tmp != null
					...
					&& tmp[2] != null
					&& tmp[2].length() > 3
					&& !tmp[2].split(":")[1].equalsIgnoreCase("")) {// 备注填写正确
					...
					/**
					 * 关闭一笔订单，可以是主订单或子订单。当订单从创建到关闭时间小于10s的时候，会报“
					 * CLOSE_TRADE_TOO_FAST”错误。
					 * 
					 * 参考：
					 * http://api.taobao.com/apidoc/api.htm?path=cid:5-apiId:83
					 */

					Date date = new Date();
					if (date.getTime() - trade.getCreated().getTime() > 10000) {// 执行关闭
						closeOrderById(trade.getOrders());
					} else {
						Thread.sleep(10000);
						closeOrderById(trade.getOrders());
					}
	...
	/** 查询指定时间段的已付款订单 */
	public List<Trade> queryTradeList() throws Exception {
		...
		Date dateTime = SimpleDateFormat.getDateTimeInstance().parse("2012-07-17 16:31:00");
		req.setStartModified(dateTime);
		dateTime = SimpleDateFormat.getDateTimeInstance().parse("2012-07-17 16:40:00");
	...
	/** 关闭指定的订单集合 */
	public boolean closeOrderById(List<Order> orders) throws Exception {
		if (orders != null) {
